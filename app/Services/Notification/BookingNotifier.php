...
class BookingNotifier
{
     ...
     * booking. Renders each removed locker in its OWN isolated panel (the same
     * EmailTheme::panel used by the confirmation email's locker cards), instead
     * of a plain comma list, so it's clear and precise.
     *
    ...
    public static function sendLockerRemoved(Booking $booking, array $removedNumbers): void
    {
        ...
        self::send($booking, 'booking_locker_removed', [
            ...
            'removed_lockers_block' => self::removedLockersBlock($removedNumbers, $locale),
        ]);
    }

    /**
     * One panel per removed locker. Split out from sendLockerRemoved() so the
     * email-rendering tests can exercise it without a live booking.
     *
     * @param  string[]  $removedNumbers
     */
    private static function removedLockersBlock(array $removedNumbers, string $locale): string
    {
        ...
        foreach ($removedNumbers as $num) {
            $blocks .= EmailTheme::panel(
                '<p style="margin:0 0 4px;color:'.EmailTheme::MUTED.';font-size:12px;text-transform:uppercase;letter-spacing:1px">'.e($label).'</p>'
                .'<p style="margin:0;font-size:34px;color:'.EmailTheme::TEXT.';font-weight:bold;letter-spacing:2px">'.e($num).'</p>'
                .'<p style="margin:8px 0 0;color:'.EmailTheme::DANGER.';font-size:12px">'.e($note).'</p>'
            );
        }
    ...
    private static function pinVars(Booking $booking, string $locale): array
    {
        ...
        foreach ($bls as $bl) {
            ...
            // The locker number is the single most important line in the whole
            // email. It is rendered white, so its dark background MUST be inline
            // on the same panel (EmailTheme::panel) — never a CSS class, or a
            // client that drops <style> shows white-on-white and the customer
            // sees an empty gap where their locker number should be.
            $blocks[] = EmailTheme::panel(
                '<p style="margin:0 0 4px;color:'.EmailTheme::MUTED.';font-size:12px;text-transform:uppercase;letter-spacing:1px">'.e($labelLocker).'</p>'
                .'<p style="margin:0 0 12px;font-size:34px;color:'.EmailTheme::TEXT.';font-weight:bold;letter-spacing:2px">'.e($number).'</p>'
                .'<p style="margin:0 0 4px;color:'.EmailTheme::MUTED.';font-size:12px">'.e($accessLabel).'</p>'
                .'<p style="margin:0;font-size:28px;color:'.EmailTheme::ACCENT.';font-weight:bold;letter-spacing:6px">'.e($pin).'#</p>'
            );
        }

        // Entry door block goes first, then one block per locker.
        $entryBlock = EmailTheme::panel(
            '<p style="margin:0 0 6px;color:'.EmailTheme::MUTED.';font-size:13px">'.e($labelEntry).'</p>'
            .'<p style="margin:0;font-size:28px;color:'.EmailTheme::ACCENT.';font-weight:bold;letter-spacing:6px">'.e($entryCode).'#</p>'
        );
