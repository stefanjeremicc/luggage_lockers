        ...
        <div v-if="tab === 'passcodes'">
            <div class="bg-[#1A1A1A] border border-[#2A2A2A] rounded-xl p-5 mb-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-5 gap-2">
                    ...
                    <input v-model="newPwd.code" @input="newPwd.code_generated = false" placeholder="Code (4-9 digits)" class="bg-[#111] border border-[#2A2A2A] rounded px-3 py-2 text-sm" />
...
// (random code) or bubble the error back so the admin picks again (typed).
const newPwd = ref({ type: 3, code: '', name: '', start: '', end: '', code_generated: false });
...
const genCode = () => {
    ...
    newPwd.value.code_generated = true;
...
const createPasscode = async () => {
    ...
    try {
        const body = {
            code: newPwd.value.code,
            type: newPwd.value.type,
            name: newPwd.value.name || null,
            // 0 = generated by us; backend may auto-retry with a fresh random
            // value on TTLock -3007 collision. 1 = admin typed; first failure
            // bubbles back so they can pick another.
            user_typed: newPwd.value.code_generated ? '0' : '1',
        };
        ...
        // Backend echoes back the final code used — show it because auto-retry
        // may have replaced the original random suggestion.
        const finalCode = data.code || newPwd.value.code;
        toast.success(`Passcode created: ${finalCode}`);
        newPwd.value.code = ''; newPwd.value.name = ''; newPwd.value.start = ''; newPwd.value.end = ''; newPwd.value.code_generated = false;
        await loadPasscodes();
    } catch (e) { toast.error(friendlyError(e.message)); }
    finally { busy.value = false; }
};
