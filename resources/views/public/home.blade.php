...
@php
    ...
    $heroTagline = $homePage?->section('hero.tagline') ?: \App\Helpers\SiteSettings::heroTagline($locale);
...
@section('head')
    {{-- Preload the LCP hero image so it paints sooner (Core Web Vitals).
         AVIF preload for modern browsers (Chrome/Edge/FF/Safari16+) — they
         pick the right variant via imagesrcset. Browsers that don't grok
         image/avif silently ignore this preload and fall through to the
         <picture> markup below, which has a WebP <source> as the fallback.
         For landing-page custom hero images we keep the original simple
...
<section class="hero-section relative flex items-center justify-center overflow-hidden -mt-20">
    {{-- Hero with multi-format responsive sources:
           AVIF: -60% desktop, -25% mobile vs WebP. Picked first by browsers
                 that support image/avif (Chrome/Edge/FF/Safari16+).
           WebP: fallback for older Safari + a few Android stock browsers.
         <picture> ensures only ONE image is loaded — never both formats.
         Same width/height attribute → CLS unchanged. --}}
    ...
    <div class="relative z-10 max-w-4xl mx-auto px-6 sm:px-6 lg:px-8 pt-24 pb-28 sm:pt-28 sm:pb-32 text-center">
        ...
        {{-- Social-proof tagline under the buttons (admin-editable, per-locale).
             Mobile: fluid font (clamp) sized as LARGE as fits on ONE line, scaling
             with viewport down to ~320px, capped at 16px on desktop. The small
             negative inline margin reclaims a bit of the hero's px-6 gutter so the
             font can go bigger; the checkmark stays hidden below sm to free width. --}}
        @if($heroTagline)
            <p class="mt-6 sm:mt-7 flex items-center justify-center gap-2 whitespace-nowrap font-medium text-white/70"
               style="margin-inline:-0.75rem;font-size:clamp(12px,3.7vw,16px)">
                <svg class="hidden sm:inline sm:w-[18px] sm:h-[18px] text-[#F59E0B] shrink-0" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true"><path fill-rule="evenodd" d="M16.704 4.153a.75.75 0 01.143 1.052l-8 10.5a.75.75 0 01-1.127.075l-4.5-4.5a.75.75 0 011.06-1.06l3.894 3.893 7.48-9.817a.75.75 0 011.05-.143z" clip-rule="evenodd"/></svg>
                {{ $heroTagline }}
            </p>
        @endif
    </div>
...
<section class="py-16 lg:py-24">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
        @if($locale === 'sr')
            <h2 class="text-3xl font-bold mb-5">Čuvanje prtljaga u Beogradu</h2>
            <p class="text-[#A0A0A0] leading-relaxed mb-4">Tražiš sigurno i povoljno čuvanje prtljaga u Beogradu? Naši pametni samouslužni ormarići u centru grada čuvaju torbe 0–24, pa razgledaš Beograd bez tereta — pre prijave u hotel, dok čekaš let ili autobus, ili kad jednostavno želiš ruke slobodne.</p>
            <p class="text-[#A0A0A0] leading-relaxed">Smešteni smo par minuta od <strong class="text-[#E5E5E5]">Slavije</strong>, glavne autobuske stanice i železničke stanice Prokop, a lako dostupni i sa A1 aerodromskog busa. Knez Mihailova, Trg republike i Kalemegdan su tu blizu — ostaviš torbe i odmah si u centru zbivanja.</p>
        @else
            <h2 class="text-3xl font-bold mb-5">Luggage storage in Belgrade</h2>
            <p class="text-[#A0A0A0] leading-relaxed mb-4">Looking for secure, affordable luggage storage in Belgrade? Our smart self-service lockers in the city centre keep your bags safe 24/7, so you can explore Belgrade hands-free — before hotel check-in, while waiting for a flight or bus, or whenever you just want your hands free.</p>
            <p class="text-[#A0A0A0] leading-relaxed">We're minutes from <strong class="text-[#E5E5E5]">Slavija Square</strong>, the main bus station and Prokop railway station, and easy to reach from the A1 airport shuttle. Knez Mihailova street, Republic Square and Kalemegdan Fortress are all close by — drop your bags and you're right in the middle of everything.</p>
        @endif
    </div>
</section>
@endif
