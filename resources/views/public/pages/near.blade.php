...
<section class="py-16 lg:py-24">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <nav class="text-xs text-[#A0A0A0] mb-4">
            <a href="{{ route($lp.'home') }}" class="hover:text-[#F59E0B]">{{ __('Home') }}</a>
            <span class="mx-2">·</span>
            <span>{{ __('Near :poi', ['poi' => $poiName]) }}</span>
        </nav>
                   ...
                   class="card flex items-start justify-between gap-4 hover:border-[#F59E0B] transition">
                    ...
                    @if(isset($loc->distance_km))
                        <div class="text-right shrink-0">
                            ...
                            <div class="text-xs text-[#A0A0A0]">{{ __('as the crow flies') }}</div>
                        </div>
                    @endif
                </a>
            @empty
        ...
        <div class="mt-12 card">
            <h3 class="text-lg font-semibold text-white mb-2">{{ __('How to book') }}</h3>
            <p class="text-sm text-[#A0A0A0]">
                {{ __('Pick a location, book online in 60 seconds, and pay cash on arrival.') }}
            </p>
        </div>
    </div>
</section>
