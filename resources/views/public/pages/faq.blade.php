...
<section class="py-16 lg:py-24">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
        <h1 class="text-3xl sm:text-4xl font-bold text-center mb-4">{{ __('Frequently Asked Questions') }}</h1>
        ...
        <div class="mb-8">
            @if($category)
            <h2 class="text-lg sm:text-xl font-semibold mb-4 text-[#F59E0B]">{{ $category }}</h2>
            @endif
            <div class="space-y-3">
                @foreach($categoryFaqs as $faq)
                <div class="card !p-0 overflow-hidden" x-data="{ open: false }">
                    <button @click="open = !open" class="w-full flex items-center justify-between text-left py-4 px-4 sm:py-5 sm:px-6 cursor-pointer">
                    ...
                    <div x-show="open" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 -translate-y-1" x-transition:enter-end="opacity-100 translate-y-0" x-cloak
                         class="px-4 pb-4 sm:px-6 sm:pb-5 text-[#A0A0A0] text-sm leading-relaxed border-t border-[#2A2A2A]">
